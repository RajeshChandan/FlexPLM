 ...
 *
 */
public class SMSEPDProdSampleWFEmailQueueInitializer {
	/**
	 * 
	 * Declaration for LOGGER.
	 */
	private static final Logger LOGGER = Logger.getLogger(SMSEPDProdSampleWFEmailQueueInitializer.class);
	/**
	 ...
	 * Constructor.
	 */
	protected SMSEPDProdSampleWFEmailQueueInitializer() {
		// protected Constructor SMSEPDProdSampleWFEmailQueueInitializer()
	}
	...
	public static void main(String args[]) throws WTException {
		LOGGER.debug("### SMSEPDProdSampleWFEmailQueueInitializer.main() - Start ###");
		...
		if (args.length < 1) {
			LOGGER.debug(
					"Usage: windchill com.sportmaster.wc.emailutility.queue.sepd.SMSEPDProdSampleWFEmailQueueInitializer [options]\n");
			LOGGER.debug(
		...
		try {
			...
			// during scheduled run time
				...
				timeStamp = SMUtill.getTimestamp(
						SMEmailUtilConstants.SEPD_PRODUCT_SAMPLE_WF_EMAIL_SCHEDULE_QUEUE_START_TIME,
						SMEmailUtilConstants.TIME_ZONE,
						SMEmailUtilConstants.SEPD_PRODUCT_SAMPLE_WF_EMAIL_SCHEDULE_QUEUE_START_AM);
			} // If neither then should not run
			else {
				return;
			}
			LOGGER.debug("Product Sample Workflow notification scheduled timestamp==" + timeStamp);
			...
			smNotificationServiceManager.scheduleJobForNotification(
					SMEmailUtilConstants.SEPD_PRODUCT_SAMPLE_WF_EMAIL_SCHEDULE_QUEUE_NAME, true,
					"executeSEPDProdSampleNotificationQueueEntry",
					SMSEPDProdSampleWFEmailQueueInitializer.class.getName(), timeStamp);
			...
			LOGGER.error(SMEmailUtilConstants.QUEUE_START_ERROR_CODE + SMEmailUtilConstants.QUEUE_START_ERROR_MESSAGE
					+ SMEmailUtilConstants.SEPD_PRODUCT_SAMPLE_WF_EMAIL_SCHEDULE_QUEUE_NAME);
			LOGGER.error(errorMsg);
			wtExp.printStackTrace();
		}
		LOGGER.debug("### SMSEPDProdSampleWFEmailQueueInitializer.main() - End ###");
	}
	 ...
	 */
	public static synchronized StatusInfo executeSEPDProdSampleNotificationQueueEntry() {
		LOGGER.debug(
				"### SMSEPDProdSampleWFEmailQueueInitializer.executeSEPDProdSampleNotificationQueueEntry() - Start ###");
		Timestamp scheduleTimeStamp = null;
		// Initialize StatusInfo variable
		StatusInfo statusInfo = null;
		try {

			// Calling processSEPDProdSampleScheduleQueue to send email notifications for
			// opentasks in last 24 hours.
			SMSEPDProdSampleEmailProcessor.processSEPDProdSampleScheduleQueue();
			...
			try {
				...
				LOGGER.debug(
						"#######################   SEPD product Sample WF NOTIFICATION SCHEDULE QUEUE ENDS  ######################");
		...
		LOGGER.debug(
				"### SMSEPDProdSampleWFEmailQueueInitializer.executeSEPDProdSampleNotificationQueueEntry() - End ###");
		return statusInfo;
	}
