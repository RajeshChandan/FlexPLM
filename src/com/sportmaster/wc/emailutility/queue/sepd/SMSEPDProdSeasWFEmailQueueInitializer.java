 ...
 *
 */
public class SMSEPDProdSeasWFEmailQueueInitializer {
	/**
	 * 
	 * Declaration for LOGGER.
	 */
	private static final Logger LOGGER = Logger.getLogger(SMSEPDProdSeasWFEmailQueueInitializer.class);
	/**
	 ...
	 * Constructor.
	 */
	protected SMSEPDProdSeasWFEmailQueueInitializer() {
	...
	public static void main(String args[]) throws WTException {
		LOGGER.debug("### SMSEPDProdSeasWFEmailQueueInitializer.main() - Start ###");
		...
		if (args.length < 1) {
			LOGGER.debug(
					"Usage: windchill com.sportmaster.wc.emailutility.queue.sepd.SMSEPDProdSeasWFEmailQueueInitializer [options]\n");
			LOGGER.debug(
		...
		try {
			...
			// during scheduled run time
				...
				scheduleTimeStamp = SMUtill.getTimestamp(
						SMEmailUtilConstants.SEPD_PS_WF_EMAIL_SCHEDULE_QUEUE_START_TIME, SMEmailUtilConstants.TIME_ZONE,
						SMEmailUtilConstants.SEPD_PS_WF_EMAIL_SCHEDULE_QUEUE_START_AM);
			} // If neither then should not run
			else {
				return;
			}
			LOGGER.info("Product Season Workflow notification scheduled timestamp==" + scheduleTimeStamp);
			...
			smNotificationServiceManager.scheduleJobForNotification(
					SMEmailUtilConstants.SEPD_PS_WF_EMAIL_SCHEDULE_QUEUE_NAME, true,
					"executeSEPDProdSeasNotificationQueueEntry", SMSEPDProdSeasWFEmailQueueInitializer.class.getName(),
					scheduleTimeStamp);
			...
			LOGGER.error(SMEmailUtilConstants.QUEUE_START_ERROR_CODE + SMEmailUtilConstants.QUEUE_START_ERROR_MESSAGE
					+ SMEmailUtilConstants.SEPD_PS_WF_EMAIL_SCHEDULE_QUEUE_NAME);
			LOGGER.error(errorMsg);
			wtExp.printStackTrace();
		}
		LOGGER.debug("### SMSEPDProdSeasWFEmailQueueInitializer.main() - End ###");
	}
	 ...
	 */
	public static synchronized StatusInfo executeSEPDProdSeasNotificationQueueEntry() {
		LOGGER.debug(
				"### SMSEPDProdSeasWFEmailQueueInitializer.executeSEPDProdSeasNotificationQueueEntry() - Start ###");
		...
		try {
			...
			SMSEPDProdSeasEmailProcessor.processSEPDProdSeasScheduleQueue();
			
			// Setting interval time for queue rescheduling
			scheduleTimeStamp = SMUtill.getTimestamp(SMEmailUtilConstants.SEPD_PS_WF_EMAIL_SCHEDULE_QUEUE_START_TIME,
					SMEmailUtilConstants.TIME_ZONE, SMEmailUtilConstants.SEPD_PS_WF_EMAIL_SCHEDULE_QUEUE_START_AM);
			...
			try {
				...
				LOGGER.debug(
						"#######################   SEPD PRODUCT SEASON WF NOTIFICATION SCHEDULE QUEUE ENDS  ######################");
		...
		LOGGER.debug("### SMSEPDProdSeasWFEmailQueueInitializer.executeSEPDProdSeasNotificationQueueEntry() - End ###");
		return statusInfo;
	}
