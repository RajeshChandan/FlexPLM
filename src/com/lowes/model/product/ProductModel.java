package com.lowes.model.product;
...
public class ProductModel {
    ...
    public List<LCSSeason> findSeasons(LCSProduct product) throws WTException {
        ...
        Collection<?> results = (new LCSSeasonQuery()).findSeasons(ObjectUtil.getLatestIteration(product));
    ...
    /**
     * Save all product records, this used translation, while error found - return false and rollback changes.
     *
     * @param products - list of LCSProduct
     * @return - boolean
     */
    public boolean saveAllProducts(List<LCSProduct> products) {
        boolean status = false;
        Transaction trx = new Transaction();
        try {
            LCSProductLogic productLogic = new LCSProductLogic();
            productLogic.saveAllProducts(products);
            status = true;
        } catch (Exception e) {
            trx.rollback();
            logger.error(e);
    ...
    public void deleteProduct(LCSProduct product) throws WTException {
        ...
        LCSProductLogic productLogic = new LCSProductLogic();
        productLogic.delete(ObjectUtil.getLatestIteration(product), true);
        ...
        if (results.getResultsFound() > 0) {
            ...
            results.getResults().forEach((Consumer<Object>) o -> {
                try {
                    ...
                    LCSProduct product = (LCSProduct) LCSQuery.findObjectById("VR:com.lcs.wc.product.LCSProduct:" + flexObj.getString(LCSPRODUCT_BRANCHIDITERATIONINFO));
                    product = (LCSProduct) VersionHelper.latestIterationOf(product);
                    productMap.put(flexObj.getString(LCSPRODUCT_BRANCHIDITERATIONINFO), product);
                } catch (WTException e) {
                    logger.error(e);
                }
            });
     ...
     * @throws WTException -  wt exception
     */
    public Map<String, LCSProduct> findProduct(List<Map<String, String>> criteria, List<String> searchFields) throws WTException {
        logger.log(Level.DEBUG, "Find Product function execution Started");
        ...
        logger.log(Level.INFO, "query Statement >>{}", statement.getSqlStatement());
        //Get FlexObject from the SearchResults instance, using to form LCSProduct instance, which is needed to return from function header to calling function
        SearchResults results = LCSQuery.runDirectQuery(statement);
        logger.log(Level.DEBUG, "Result found >>{}", results.getResultsFound());
        if (results.getResultsFound() > 0) {
