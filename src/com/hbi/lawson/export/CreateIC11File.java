...
public class CreateIC11File 
{
	...
	{	LCSLog.debug("Start buildIC11Record builder "+builder);
        ...
        Transaction txn = null;
       	HbiPurchXref purchXref = null;
		HbiInvTypeXref invTypeXref = null;
        try {
            ...
            LCSLog.debug("Start buildIC11Record commit ");
            txn.commit();
            LCSLog.debug("Start buildIC11Record commit done ");
        } 
		finally 
		{
        ...
        LCSLog.debug("buildIC11Record purchXRefList "+purchXRefList);
        if(purchXRefList != null && !purchXRefList.isEmpty() && purchXRefList.size() > 0) 
        purchXref = purchXRefList.get(0);
        LCSLog.debug("buildIC11Record invTypeXRefList "+invTypeXRefList);
		if(invTypeXRefList != null && !invTypeXRefList.isEmpty() && invTypeXRefList.size()> 0)
        invTypeXref = invTypeXRefList.get(0);
	...
	//if(flexTypeIdPath.startsWith("\16052\227276068"))
                ...
                && (hbiAttrCode == null || "".equals(hbiAttrCode) || "------".equals(hbiAttrCode))) 
		{
			...
			log.info("hbiSKU FEP Default=" + hbiSKU);
			log.info("hbiStyle length FEP Default=" + hbiStyle.length());
		...
		else 
		{
			...
			log.info(" New method hbiSKU FEP =" + hbiSKU);
			log.info("hbiStyle length FEP =" + hbiStyle.length());
        }
        ...
        System.out.println("buildIC11Record dateFormat "+dateFormat.format(material.getCreatestampa2()));
	...
	public static String removeDuplicates(String iC11Build){
		log.info("CreateIC11File removeDuplicates "+iC11Build);
			...
			log.info("CreateIC11File res "+res);
			return res.toString();
	}
