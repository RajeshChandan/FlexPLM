 ...
 * @author UST
 */
public class PLMMaterialLoader implements PLMETLLoader {

    public static final String logLevel = LCSProperties.get("com.hbi.etl.logLevel");
    static Logger etlLogger = PLMETLLogger.createInstance(PLMMaterialLoader.class, logLevel);
    ...
    public void load(Vector objList) {
        ...
        etlLogger.debug("Start HbiMaterial Loader");
        int i;
        for (i = 0; i < objList.size(); i++) {

            HbiMaterial transObj = (HbiMaterial) objList.elementAt(i);
            Query query = session.createQuery("from HbiMaterial where branchiditerationinfo = :branchiditerationinfo");
            ...
            if (query.uniqueResult() == null) {
                ...
                HbiMaterial updObj = (HbiMaterial) query.uniqueResult();
                transObj.setLoadercreatetime(updObj.getLoadercreatetime());
                transObj.setPrimarykey(updObj.getPrimarykey());
                System.out.println(" Material Data exists GO update");
        ...
        txn.commit();
        if(txn.wasCommitted()){
            etlLogger.info("HbiMaterial Loaded Successfully!");
        } else {
            txn.rollback();
            etlLogger.info("HbiMaterial Load Failed!");
        }         
        session.close();
    }

    public void update(HbiMaterial obj) {
