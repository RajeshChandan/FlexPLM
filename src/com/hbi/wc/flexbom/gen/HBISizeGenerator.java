 ...
 * @author  Chuck
 */
public class HBISizeGenerator extends BomDataGenerator {
...
//    private static final String COLORREFID = "IDA3D5";
    ...
    private boolean individualSizesRequired=true;
...
/* Added for CA # 318-13 -- End */
    ...
    //Holds the List of BOM types for which individual sizes not needed
    public String BOM_TYPES_IND_SIZES_NOT_NEEDED=LCSProperties.get("com.hbi.wc.flexbom.gen.HBISizeGenerator.BOMTypesForWhichINdividualSizesNotNeeded","Materials\\HBI\\Selling\\Pack Case");
    
    public boolean DEBUG = LCSProperties.getBoolean("com.lcs.wc.flexbom.gen.SizeGenerator.verbose");
   ...
   /* public Double partNameWidth = (new Float(LCSProperties.get("com.lcs.wc.flexbom.gen.SizeGenerator.partNameWidth", "1.5"))).doubleValue();//.floatValue();
    public Double materialNameWidth = (new Float(LCSProperties.get("com.lcs.wc.flexbom.gen.SizeGenerator.materialNameWidth", "1.25"))).doubleValue();//.floatValue();
    public Double supplierNameWidth = (new Float(LCSProperties.get("com.lcs.wc.flexbom.gen.SizeGenerator.supplierNameWidth", "1.25"))).doubleValue();//.floatValue();
    public Double sizeWidth = (new Float(LCSProperties.get("com.lcs.wc.flexbom.gen.SizeGenerator.sizeWidth", "0.75"))).doubleValue();//.floatValue();
    */
    ...
    public Collection getTableColumns() throws WTException {
        ...
        if(this.view != null){
            ...
            String colName = "";
            /*
            Iterator it = viewAtts.iterator();
            while(it.hasNext()){
                colName = (String)it.next();
                if(colName.startsWith("BOM")){
                    it.remove();
                }
            }
            */
        ...
        viewColumns.put("materialDescription", column);
        	//Code Upgrade Wipro Team
        	String hbiMaterialSKUCol = materialType.getAttribute("hbi17DigitSKU").getAttributeName();
        	column = new TableColumn();
            column.setHeaderLabel("HBI MATERIAL SKU");
            //column.setTableIndex("LCSMATERIAL.ATT86");
            column.setTableIndex("LCSMATERIAL." +hbiMaterialSKUCol);
            column.setDisplayed(true);
            column.setPdfColumnWidthRatio(materialNameWidth);
           // column.setLinkMethod("viewMaterial");
           ...
           // ((BOMMaterialTableColumn)column).setDescriptionIndex("FLEXBOMLINK." + MAT_DESCRIPTION_ATT);
            viewColumns.put("LCSMATERIAL."+hbiMaterialSKUCol, column);
		...
		*/
        ...
        //Sizes part - Venkat
        //System.out.println("individualSizesRequired---------->"+individualSizesRequired);
        
       //System.out.println("this.sizes---------->"+this.sizes);
        
        if(individualSizesRequired){
...
//Modified for CA # 318-13 - to consider colorway BOM attributes
        	...
        	String reportKey=(String)params.get("REPORT_KEY");
            LCSProduct product=(LCSProduct)LCSQuery.findObjectById((String)params.get("PRODUCT_ID"));
            
        	//LCSProduct product=(LCSProduct)LCSQuery.findObjectById(params.get("PRODUCT_ID"));
        
            
            if(BOM_TYPES_IND_SIZES_NOT_NEEDED.contains(type.getFullName()) && "BASIC CUT & SEW - SELLING".equals(product.getFlexType().getFullName()) && "sizingReport".equals(reportKey) ){
            	
            	 // System.out.println("Criteria Matched ,PLease disable -----");
            	individualSizesRequired=false;
            	
            	//((LCSProduct)LCSQuery.findObjectById(params.get("PRODUCT_ID"))).getFlexType().getFullName().contains("SELLING");
            }
   ...
   /*
    private Map geDatatRow(Collection data, String key, String value){
        if(data != null && FormatHelper.hasContent(key) && FormatHelper.hasContent(value)){
            Iterator i = data.iterator();
            Map dm = null;
            while(i.hasNext()){
                dm = (Map)i.next();
                if(dm.get(key).equals(value)){
                    return dm;
                }
            }
        }
