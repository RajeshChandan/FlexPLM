...
public class HBICreateNewColumnFromSizeCattoSizes 
{
	public static void main(String[] args)
	{
		try 
		{
			FileInputStream file = new FileInputStream(new File("D:\\SP_SizeCat_gen.xlsx"));
			...
			while (rowIterator.hasNext()) 
			{
				...
				al.add(row.getCell(5).toString());
				keyValMap.put(row.getCell(0).toString() , al);
			}
			file.close();
			
			FileInputStream file_1 = new FileInputStream(new File("D:\\SP_SIZES.xlsx"));
				...
				if (keyValMap.containsKey(row_1.getCell(0).toString()))
					{
						...
						row_1.getCell(7).setCellValue(val.get(4));
					}
				}
				file_1.close();

				FileOutputStream outputStream_1 = new FileOutputStream(new File("D:\\SP_SIZES_gen.xlsx"));
