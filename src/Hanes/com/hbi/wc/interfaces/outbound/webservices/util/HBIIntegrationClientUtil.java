...
public class HBIIntegrationClientUtil
{
	...
	public boolean getTransactionObjectInitializationStatusFor(String transactionObjUniqueIdentifier, String flexObjectClassName) throws WTException, WTPropertyVetoException
	{
		...
		String id = transactionObjectFlexType.getTypeIdPath();
				//FormatHelper.getNumericObjectIdFromObject((WTObject)transactionObjectFlexType);
		String uniqueIdentifierDBColumn = transactionObjectFlexType.getAttribute(HBIProperties.hbiFlexObjectUniqueIdentifierKey).getColumnDescriptorName();
        ...
        statement.appendAndIfNeeded();
        statement.appendCriteria(new Criteria(new QueryColumn(LCSLifecycleManaged.class, "flexTypeIdPath"), "?", "="), id);
		...
		if(measurementsObj != null)
		{
			...
			if("INSTANCE".equalsIgnoreCase(measurementType) && "update".equalsIgnoreCase(eventType))
			{
				...
				if(measurementComponentsCollection != null && measurementComponentsCollection.size() > 0 )
				{
					...
					/*Added by Sobabu on 8/20/2015 for NullPointerException 151288-15*/
	...
	public Collection<FlexObject> getHBITransactionBusinessObjectForPendingStatus(String flexObjectClassName) throws WTException
	{
		...
		String typeIdPath = transactionObjectFlexType.getTypeIdPath();
		String flexObjectClassNameDBColumn = transactionObjectFlexType.getAttribute(HBIProperties.hbiFlexObjectClassNameKey).getColumnDescriptorName();
		String transactionStatusDBColumn = transactionObjectFlexType.getAttribute(HBIProperties.hbiTransactionStatusKey).getColumnDescriptorName();
