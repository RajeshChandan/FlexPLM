...
* 
*
*       This class is implemented by taking the code from HBIMatColorPDFContent and
*       modified as per requirement
* 
*/

public class HBIGPColorVariationPDFContent extends BOMPDFContentGenerator {

    private static final boolean DEBUG = LCSProperties.getBoolean("com.lcs.wc.flexbom.gen.HBIGPColorVariationPDFContent.verbose");
    private static final int DEBUG_LEVEL = Integer.parseInt(LCSProperties.get("com.lcs.wc.flexbom.gen.HBIGPColorVariationPDFContent.verboseLevel", "1"));
    //private static final String CLASSNAME = HBIGPColorVariationPDFContent.class.getName();
    public float tableWidthPercent = (new Float(LCSProperties.get("com.lcs.wc.flexbom.gen.HBIGPColorVariationPDFContent.tableWidthPercent", "95.0"))).floatValue();
	...
	public static final String  VALID_BOM_TYPES = LCSProperties.get("bomreport.color.garmentproduct.bomtypes");
	public static final String LABEL_TYPE = "Product\\HBI-SUPPORTING\\LABEL";

    /** Creates a new instance of HBIGPColorVariationPDFContent */
    public HBIGPColorVariationPDFContent() {
    }
	...
	public Collection getPDFContentCollection(Map params, Document document) throws WTException {
        if(DEBUG){ debug("MatColorPDFContent.getPDFContentCollection");}
		...
		//GP teck pack customizations - 09/20/2018
		boolean gpBOMTechpackStatus = isValidateGPBOMTypeForBOMReport(bomPart);
		if(gpBOMTechpackStatus)
		{
	        Map tparams = new HashMap(params.size() + 2);
	        tparams.putAll(params);
	
	        HBIGPColorVariationGenerator bomDG = new HBIGPColorVariationGenerator();
	        ...
	        while(sectionIter.hasNext()) {
	            ...
	            while(skusIt.hasNext()) {
	                ...
	                bomDG = new HBIGPColorVariationGenerator();
	                bomDG.init(tparams);
...
//		LCSPart lcsPart = (LCSPart)VersionHelper.latestIterationOf(wtPartMaster);
		...
		if(lcsPart instanceof LCSProduct)
		{
			...
			if((BASIC_CUT_AND_SEW_GARMENT.equalsIgnoreCase(productFlexTypePath)||LABEL_TYPE.equalsIgnoreCase(productFlexTypePath)) && !Arrays.asList(VALID_BOM_TYPES.split(",")).contains(bomPartFlexTypePath))
			{
				debug(" <<<<<< MATERIAL COLOR bomPart >>>>>>>" +bomPart.getName());
