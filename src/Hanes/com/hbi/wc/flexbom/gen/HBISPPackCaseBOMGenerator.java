 ...
 * 
 *
 *       This class generates the Pack Case BOM specification details.
 *       It generates the table columns and the data that needs to be 
 *       displayed in the Pdf Page
 * 
 */
public class HBISPPackCaseBOMGenerator extends BomDataGenerator implements Comparator<String>{
...
//	private String MAT_DESCRIPTION_ATT = "ATT3";
	...
	private FlexType bomPackCaseType = null;
	...
	public String PACKCASE_BOM_TYPE = LCSProperties.get("bomreport.packcase.salesproduct.bomtypes");
	...
	//Default constructor
	public HBISPPackCaseBOMGenerator() {
	}
	//Overloaded Constructor
	public HBISPPackCaseBOMGenerator(List<String> sortOrder) {
	...
	public Collection getTableColumns() throws WTException {
		...
		if(section.equals("packagingInstructions")||section.equals("plantToDc"))  { 
			viewAtts.clear();
			viewAtts.add("Material.hbiComment");
			...
			column.setSpecialClassIndex("CLASS_OVERRIDE");
			column.setPdfColumnWidthRatio(partNameWidth);
			viewColumns.put("Material.hbiComment", column);
	...
	public void init(Map params) throws WTException {
		...
		if(params != null){
			...
			bomPackCaseType = FlexTypeCache.getFlexTypeFromPath(PACKCASE_BOM_TYPE);
			...
			COMP_NAME_DISPLAY = bomPackCaseType.getAttribute("partName").getAttDisplay();
			COMP_NAME_ATT = bomPackCaseType.getAttribute("partName").getVariableName();
	...
	private Collection processBranch1(FlexObject topLevel, Map <String, Collection> params) throws WTException{
		...
		if(dimRows.isEmpty()){
			if(((section).equals("packaging")||(section).equals("casing")) && (sizeArray.size()>0) && !(colorArray.size()>0)){
				Collection<FlexObject> extraSizes = new ArrayList();
				extraSizes = extraSizeRows(topLevel,sizeArray);
				allRows.addAll(extraSizes);
			}else
			if(((section).equals("packaging")||(section).equals("casing")) && (colorArray.size()>0) && !(sizeArray.size()>0)){
		...
		if (!dimRows.isEmpty()) {
			...
			if(remainingSizes.size()>0 || remainingColors.size()>0){
				...
				}else if(((section).equals("packaging")||(section).equals("casing")) && (colorArray.size()>0) && !(sizeArray.size()>0)){
	 ...
	 * @param topLevel
	 * @param dimRows
	 * @method This method merges the Size and Color rows and present it as a single row in the report.
	 * @return
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
