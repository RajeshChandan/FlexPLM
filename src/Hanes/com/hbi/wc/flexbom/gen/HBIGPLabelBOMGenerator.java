 ...
 * 
 */
public class HBIGPLabelBOMGenerator extends BomDataGenerator {
...
/* Added for CA # 318-13 -- end*/
    ...
    public float thumbNailWidth = (new Float(LCSProperties.get("com.lcs.wc.flexbom.gen.MatColorGenerator.thumbNailWidth", "2.0"))).floatValue();
    public float applicationWidth = (new Float(LCSProperties.get("com.lcs.wc.flexbom.gen.MatColorGenerator.applicationWidth", "2.0"))).floatValue();
    public float padPrintColorWidth = (new Float(LCSProperties.get("com.lcs.wc.flexbom.gen.MatColorGenerator.padPrintColorWidth", "0.75"))).floatValue();
	...
	private FlexType matColorType = null;
	 ...
	 public static final String BASIC_CUT_AND_SEW_GARMENT = LCSProperties.get("hbi.gp.product.type");
	 public static final String MATERIAL_HBI_ATTRIBUTION_BOM = LCSProperties.get("hbi.gp.attributionbom.type");
	 public static final String MATERIAL_HBI_LABEL_BOM = LCSProperties.get("hbi.gp.labelbom.type");
	 public static final String GP_ROUTING_BOM= LCSProperties.get("hbi.gp.routingbom.type","BOM\\Materials\\HBI\\Garment Routing BOM");
	...
	private static final String LABEL_MATERIAL_TYPE_PATH = LCSProperties.get("hbi.labelreport.materialtype","Material\\Garment Label");;
	 private FlexType label_bomType = null;
	 private FlexType label_materialType = null;
	 //End HBI GP tech Pack Customizations
	
    public HBIGPLabelBOMGenerator() {
    }
	...
	public Collection getTableColumns() throws WTException {
        ...
        // viewAtts.add(0, "materialDescription");
		 // for hbi don't want Destination on Label BOM on Tech pack -Jey
        ...
        flexatt = label_materialType.getAttribute("hbiApplication");
        column = flexg.createTableColumn(null, flexatt, label_materialType, false, "LCSMATERIAL");
        column.setPdfColumnWidthRatio(applicationWidth);
        viewColumns.put("Material.hbiApplication", column);
        
        flexatt = label_materialType.getAttribute("hbiPadPrintInkColors");
        column = flexg.createTableColumn(null, flexatt, label_materialType, false, "LCSMATERIAL");
        column.setPdfColumnWidthRatio(padPrintColorWidth);
        viewColumns.put("Material.hbiPadPrintInkColors", column);
	...
	public void init(Map params) throws WTException
	{
        ...
        if(params != null){
            ...
            label_bomType = FlexTypeCache.getFlexTypeFromPath(part.getFlexType().getFullName(true));
            label_materialType = FlexTypeCache.getFlexTypeFromPath(LABEL_MATERIAL_TYPE_PATH);
            ...
            COMP_NAME_DISPLAY = label_bomType.getAttribute("partName").getAttDisplay();
			...
			if (FormatHelper.hasContent(product_Oid)) {
				...
				if ((product != null) && product.getFlexType().getFullName(true).equals(BASIC_CUT_AND_SEW_GARMENT)) {
            ...
            if(this.dataSet != null){
               ...
               debug("dataSet:: "+dataSet.size());
                ...
                while(branchIter.hasNext()){
                    ...
                    debug("isSubComponent :: "+isSubComponent);
                    debug("isComplexMaterial :: "+isComplexMaterial);

                    if (!isSubComponent)
					{
						if (isComplexMaterial) {
							processedData.addAll(processComplexBranch(topLevelBranch, params));
						} else {
							processedData.addAll(processBranch(topLevelBranch, params));
						}
					}
    ...
    */
	private boolean isBOMGPAttribution(FlexBOMPart part) {
		if ((part != null) && part.getFlexType().getFullName(true).equals(MATERIAL_HBI_ATTRIBUTION_BOM)) {
			return true;
		}
		return false;
	}
 ...
 */
	private boolean isBOMGPRoutingTable(FlexBOMPart part) {
		if (part != null && part.getFlexType().getFullName(true).equals(GP_ROUTING_BOM)) {
			return true;
		}
		return false;
	}   
...
private Collection processBranch(FlexObject topLevel,Map params) throws WTException
   {
		...
		else if (topLevelMci.hasMaterialInfo())
		{
			...
			calculateSectionAttributes(newRow);
			data.add(newRow);
			debug("topLevelMci.hasMaterialInfo():: "+data.size());
		}
		else if (topLevelMci.hasColorInfo())
		{
			...
			calculateSectionAttributes(newRow);
			data.add(newRow);
			debug("topLevelMci.hasColorInfo() :: "+data.size());
		}
		if(data.size()==0){
			FlexObject newRow = topLevel.dup();	
			//calculatePrice(newRow);
		...
		debug("Before sortedData :: "+data.size());
		Collection sortedData = sortBranchDataSet(data);
		debug("After sortedData :: "+data.size());
		...
		debug("processed sorted data ::"+sortedData.size());
		//sortedData =  new HBILabelBOMTechPackUtil().processFlexObjectForGarLabelBOM(sortedData,params);
		return sortedData;
    }
  ...
  */
     ...
     */
    @SuppressWarnings({ "rawtypes", "unchecked", "unused" })
	private Collection processComplexBranch(FlexObject topLevel,Map params) throws WTException{
       ...
       Collection topComponents = processBranch (topLevel,params);
            ...
            if (!bomParts.isEmpty())
            {
               ...
               for (Iterator topLevelIter = topLevelBranches.iterator(); topLevelIter.hasNext();)
               {
                      ...
                      FormatHelper.hasContent(partName)*/)
                  {
					 ...
					 //Added below line to copy the Usage per Doz value in Master Material Line to each of part specific lines -START
					 String Usage_ATT = "FLEXBOMLINK." + label_bomType.getAttribute("hbiUsage").getVariableName();
					 newRow.put(Usage_ATT, topComponent.getString(Usage_ATT));
					 //END
                     blankAllColorData(newRow);
             ...
             subComponentsList = SortHelper.sortFlexObjectsByNumber(subComponentsList, "FLEXBOMLINK.SORTINGNUMBER");
            /* Iterator itr = subComponentsList.iterator();
             while(itr.hasNext()){
            	 FlexObject log = (FlexObject) itr.next();
            	 System.out.println("[MASTERBRANCHID :: "+log.getData("FLEXBOMLINK.MASTERBRANCHID")+"], "
            	 		+ "[SORTING :: "+log.getData("FLEXBOMLINK.SORTINGNUMBER")+"], "
            	 				+ "[BRANCHID :: "+log.getData("FLEXBOMLINK.BRANCHID")+"]" );
             }
             */
       ...
       processedData =  new HBILabelBOMTechPackUtil().processFlexObjectForGarLabelBOM(processedData,params);
       
       debug("complex processed sorted data ::"+processedData.size());
 ...
 //        Collection allSources =  LCSSourcingConfigQuery.getSourcingConfigForProduct(productMaster);
	...
	private void addAllSubComponentsColorData(FlexObject subRow, FlexObject topRow, FlexBOMPart bomPart) throws WTException {
      ...
      for (Iterator skuIter = skus.iterator(); skuIter.hasNext();)
      {
         ...
         String colorHex = topRow.getString(skuId + "." + COLORHEX);
         if (FormatHelper.hasContent (matColorId) && (!"X".equals(colorName)))
         {  //Modified to add Color Name and Color HEX on the sub component rows from Master - Jeyaganeshan 
            addComplexColorData(bomPart, subRow, skuId, matColorId, colorName, colorHex);
         }
     ...
     * Added color name from Master Row -April 2019 - as part of Label BOM Report Changes -Jeyaganeshan
     */
    @SuppressWarnings("rawtypes")
	private void addComplexColorData(FlexBOMPart bomPart, FlexObject row, String skuId, String matColorId,String colorName,String colorHex) throws WTException {
       ...
       { 	
          ...
          String subComponentColorName = subComponentColorMci.colorName;
          if(FormatHelper.hasContent(subComponentColorName)) {
        	  row.put(skuId + "." + COLORNAME, subComponentColorName);
          }else if(FormatHelper.hasContent(colorName) && !"N/A".equalsIgnoreCase(colorName) ) {
        	  row.put(skuId + "." + COLORNAME, colorName);
          }else {
        	  row.put(skuId + "." + COLORNAME, subComponentColorName);
          }
          ...
          if(FormatHelper.hasContent(subComponentColorMci.colorId)){
             ...
             if(color.getThumbnail() != null){
              ...
              //Start-Jeyaganeshan
        	  if(FormatHelper.hasContent(colorHex) && !"FFFFFF".equalsIgnoreCase(colorHex)) {
        		  row.put(skuId + "." + COLORHEX, colorHex);
        	  }
	...
	private void addDimensionsData(FlexObject row, Map dimensionSubSet) throws WTException{
         ...
         if(dimensionSubSet.get(MCIDimensionHelper.SIZE1) != null)
         {
             ...
             for (Iterator iter = size1List.iterator(); iter.hasNext();)
             {
                ...
                //sizestring = addString(size1string, size1);
